...
export default function AppProviderLayoutService({matches}:Route.ComponentProps){
    ...
    const instDataEnloade = useMemo(()=> {
        if(!reqParam) return;
        return new BuildParamLoaded(st, reqParam).evaluate()
    },[st, reqParam]);
    
    console.log(instDataEnloade?.param, dataEnloaded?.param);
    console.log(instDataEnloade?.sheetToBeParam, dataEnloaded?.notProvidedState)
