...
import type { Route } from "./+types/sub-asessment";
import PermissionFeature from "~/lib/permission-feature";
import { subFiturPenilaian } from "~/features/subfitur-penilaian";
...
import MainModalPenilaian from "~/controllers/assesmen-penilaian/modals/main-modal-penilaian";


export default function SubPenilaianLayout({matches}:Route.ComponentProps) {
    
    const user = useAppSelector(state=> state.auth.user);
    const active = useAppSelector(state=>state.loadedApi.loaded);
    const user_permission = user?.permission;
    const result = PermissionFeature(user_permission ?? [], 'view menu penilaian', subFiturPenilaian);
    ...
    return (
        <AppSidebar showExport={showExport} desktopHeader="relative" title={"Penilaian - "+ titleTambahan} controlKelas={controlKelas}>
