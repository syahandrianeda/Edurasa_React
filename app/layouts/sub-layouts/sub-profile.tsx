...
import type { Route } from "./+types/sub-profile";
import { SubProfileSidebar } from "~/features/profile";

export default function SubProfileLayout({matches}:Route.ComponentProps) {
    
    const user = useAppSelector(state=> state.auth.user);
    const active = useAppSelector(state=>state.loadedApi.loaded);
    const user_permission = user?.permission;
    const result = PermissionFeature(user_permission ?? [], 'view profile', SubProfileSidebar);
    ...
    return (
        <AppSidebar showExport={showExport} desktopHeader="relative" title={"Profil-ku - "+ titleTambahan} controlKelas={controlKelas}>
