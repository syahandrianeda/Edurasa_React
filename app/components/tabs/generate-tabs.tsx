...
export default function GenerateTabs({defaultValue, tabList, contentList}:TabsConfigProps){
    return (
...
export function GenerateTabsForModal({defaultValue, tabList, contentList}:TabsConfigProps){
    return (
        <Tabs defaultValue={defaultValue} className="gap-0" >
                <TabsList className="flex-nowrap w-full md:w-fit ps-4 min-[412px]:ps-0 overflow-x-auto scrol-h-custom pb-1 md:pb-0">
                    {
                        tabList.map((m, index)=>(
                            <TabsTrigger key={index} value={m.value}>{m.label}</TabsTrigger>
                        ))
                ...
                <div className="bg-linear-to-br from-sky-300 to-sky-200 dark:from-sky-700 dark:to-sky-600 h-[calc(100vh-14.25rem)] w-full overflow-y-scroll scrol-h-custom">
                    {
                        contentList.map((m,index)=>(
                            <TabsContent key={index} value={m.value} className="flex flex-col gap-0 w-full px-4 text-sm">{m.element}</TabsContent>
                        ))
                    }
