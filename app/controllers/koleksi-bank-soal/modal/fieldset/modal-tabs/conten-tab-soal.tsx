...
export default function ContentTabSoal({currentData, 
    valueJsonPertanyaan,
    actionPertanyaan,
    valueJsonStimulus, 
    actionStimulus,
                ...
                <tbody>
                    <TRowEdura>
                        ...
                        <TdEdura className='border-0'>
                            <div className='relative mt-4'>
                                <div className='ps-1 pe-4 absolute text-[10px] -top-1 -translate-y-1/2 w-fit rounded-tr-2xl bg-sky-200 dark:bg-gray-300 '>Edit Kelas</div>
                                <Select
                                    value={kelas.toString()}
                                    onValueChange={handleSelectjenjang}
                                    >
                                    <SelectTrigger className='w-full bg-sky-200 dark:bg-gray-300 dark:border-0 text-[10px] py-0'>
                    ...
                    <TRowEdura>
                        ...
                        <TdEdura className='border-0'>
                            ...
                            <div className='relative mt-4'>
                                <div className='ps-1 pe-4 absolute text-[10px] -top-1 -translate-y-1/2 w-fit rounded-tr-2xl bg-sky-200  dark:bg-gray-300 '>Edit Bentuk Soal</div>
                                <Select
                                    value={bentukSoal}
                                    onValueChange={handleChangeBentukSoal}
                                    >
                                    <SelectTrigger className='w-full bg-sky-200  dark:bg-gray-300 dark:border-0 text-[10px] py-0'>
                                        <SelectValue placeholder="Ubah Kelas"/>
                                    </SelectTrigger>
                                    <SelectContent>
                                        {
                                            ListBentukSoal.map((soal, iJenjang)=>
                                                <SelectItem  className='text-[10px]' key={iJenjang} value={soal.name}>{soal.description}</SelectItem>
                                            )
                                        }
                                    </SelectContent>
                                </Select>
