import { Label } from "~/components/ui/label";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "~/components/ui/select";
import { KlasifikasiNoSurat } from "~/domain/surat/klasifikasi-surat-permendagri";

export default function KlasifikasiSuratTemplate({value, setValue}:{value:string, setValue:(v:string)=>void}){
    const sourceSelected = KlasifikasiNoSurat.find(s=>s.value === value)
    const sourceSelectedKlasifikasi = sourceSelected?.source;
    return (
        <div className="relative mt-5 w-full ">
                <Label className="absolute dark:bg-gray-700 border-s-2 dark:border-s-0 border-t border-sky-500 bg-sky-50 dark:text-sky-100 ps-1 text-[10px] dark:border-b-0 top-0 left-0 py-0 pe-4 rounded-se-2xl -translate-y-3.5 w-fit">Index Surat (Diteruskan untuk dasar surat keluar):</Label>
                <Select
                    // disabled={disabled}
                    value={value}
                    onValueChange={(v)=>setValue(v)}
                    >
                    <SelectTrigger className="w-full dark:bg-gray-700 bg-sky-50 border-2 dark:text-white dark:border-0 border-s-2 rounded-ss-none rounded-br-none rounded-tr-2xl focus-visible:ring-0 border-sky-500 focus-visible:border-sky-400">
                        <SelectValue placeholder="Pilih Klasifikasi"/>
                    </SelectTrigger>

                    <SelectContent>
                        {
                            KlasifikasiNoSurat.map(({ value, description },i) => 
                                    <SelectItem key={i+'_'} value={value}>
                                        {description}
                                    </SelectItem>
                            )
                        }
                    </SelectContent>
                </Select>
                <div className="text-xs">Sumber: {sourceSelectedKlasifikasi}</div>
                
            </div>
    )
}