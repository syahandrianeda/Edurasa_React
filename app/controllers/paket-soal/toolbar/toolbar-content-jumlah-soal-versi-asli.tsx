...
export default function ToolbarContentJumlahSoal(){
    ...
    useEffect(()=>{
        ...
        updateExtra(draft=>{
            ...
            draft.setting = setting;
            const data = (draft.data ?? {}) as NonNullable<typeof draft.data>;
            data.forEach((item, iItem)=>{
                const session = strukturSoal.find(s=>s.dataBentukSoal.name === item.bentukSoal.name);
                const count = session?.count ?? -1;
                if(count < item.dataSoal.length){
                    item.dataSoal.slice(0, count)
    ...
    useEffect(()=>{
        updateExtra(draft=>{
            ...
            draft.data.forEach((item, index) => {
                ...
                const count = setting.count_bentuk_soal?.[index]?.count ?? 0;
