...
export default function ButtonSendMapel({data}:{data:jp_mapelApp[]}){
    ...
    return (
        <button type="button" disabled={state.isSubmitting} className="border print:hidden bg-sky-300 rounded-3xl p-2 mx-auto mt-3 flex gap-2" onClick={onSubmit}>{state.isSubmitting && <Loader size={12} className="animate-spin self-center"/>} Simpan</button>
    )
}