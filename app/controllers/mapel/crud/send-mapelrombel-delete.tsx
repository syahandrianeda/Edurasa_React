...
export default function ButtonSendMapelDelete({data, deleteIds}:{data:jp_mapelApp[], deleteIds:number}){
    ...
    const onSubmit =  async (e: React.MouseEvent<HTMLButtonElement>)=>{
        ...
        data.forEach((d, index)=>{
            ...
            if(d.idbaris === deleteIds){
                convertDto.status = 'hapus';
            }
    ...
    return (
        <button type="button" disabled={state.isSubmitting} className="border print:hidden bg-rose-400 rounded-3xl p-2 mx-auto mt-3 flex gap-2" onClick={onSubmit}>{state.isSubmitting && <Loader size={12} className="animate-spin self-center"/>} Hapus</button>
    )
}