...
import type { cellsTableCommon, TableParseCommon } from "~/export/word-edura/parser/common-comps/type-table";
import { resolveRgbaOrOklchToHex } from "~/lib/color-hack";
import { parseImageExcel } from "./parse-image-excel";

export default async function parseCommonTableExcel(table:HTMLTableElement):Promise<TableParseCommon>{
                ...
                cells:await Promise.all(
                    ...
                    .map(async (cell)=>{
                        ...
                        return {
                            ...
                            typeData:cell.dataset.contentType as 'string'|'number'|'date'|'boolean' | undefined,    
                            children: konten
                        }
                    }))
            }
...
async function walkNode(node: Node, contents: cellsTableCommon[]): Promise<void> {
    ...
    if (node instanceof HTMLImageElement) {
        ...
        const src = await parseImageExcel(node as  HTMLImageElement)
