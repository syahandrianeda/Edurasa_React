...
export default function RekapPerKelasBulanan({fokusBulan}:{fokusBulan?:BulanType}){
    ...
    const TotalPerTanggal = useCallback((tgl:Date)=>{
        const nominalTglIni =  sheetTabungan.filter(s=>isSameDay(s.time_stamp, tgl) && s.status =="");
            ...
            <TableWithScrolling>
                                ...
                                <TRowEdura key={m.siswa_id} className="even:bg-white">
                                    ...
                                    <TdEdura className="text-end font-bold">{jumlahPerBulan && formatCurrency(jumlahPerBulan)}</TdEdura>
                                    {
                                        Tgl.map((m, iii)=>{
                                            ...
                                            const warningIsi = transaksi?.kolom === 'keluar'? "text-rose-800 font-bold before:content-['-']":'';
                ...
                <tfoot>
                    <TRowEdura>
                        ...
                        <ThEdura className="text-end font-bold">{total && formatCurrency(total) ||''}</ThEdura>
                        {
                            Tgl.map((m,index)=>
                                <ThEdura key={index} style={m.style} className={m.isLibur?'bg-rose-500':'text-end'}>{TotalPerTanggal(m.date) === 0 ? '' : formatCurrency(TotalPerTanggal(m.date))}</ThEdura>
                            )
                        }
                    </TRowEdura>
                </tfoot>
