...
import { DataTabunganRekapCurrentRombel, DtoDataTabunganCurrentRombel } from "~/context-reduct/selectores/data-tabungan-selector";
...
export default function SnapshotPerSiswaBulanan({fokusBulan}:{fokusBulan?:BulanType}){
    const sheetTabungan = useAppSelector(DtoDataTabunganCurrentRombel);
        ...
        const data = useMemo(()=>{
            if(indexBulan){
                const found =  sheetTabungan.filter(s=>s.siswa_id ===  id && isSameMonth(s.time_stamp, indexBulan));
                if(found.length > 0){
                    return found;
                }
        ...
        const jumlahNOminalDihapus = useMemo(()=>{
            const foundDelete = jumlahJalan.dataBaru.filter(s=>s.status === 'hapus')
            if(foundDelete.length > 0){
                return foundDelete?.map(m=>m.masuk || m.keluar || 0)?.reduce((a,b)=>a+Number(b))
            }
            return 0
            ...
            <TableWithScrolling className="md:w-4/5 mx-auto">
                        ...
                        jumlahJalan && jumlahJalan.dataBaru.length === 0?(
                            ...
                            jumlahJalan.dataBaru.map((m, i)=>
                                <TRowEdura key={i}>
                                    ...
                                    <TdEdura className="w-35 text-end pe-1">{
                                        m.status === "hapus"?(
                                                <s className="text-rose-500 font-bold">{m.masuk && formatCurrency(m.masuk)}</s>
                                            ):(
                                                m.masuk && formatCurrency(m.masuk)
                                            )
                                    }</TdEdura>
                                    <TdEdura className="w-35 text-end pe-1">
                                        {
                                        ...
                                        m.status === "hapus"?(
                                                <s className="text-rose-500 font-bold">{m.keluar && formatCurrency(m.keluar)}</s>
                                            ):(
                                                m.keluar && formatCurrency(m.keluar)
                                            )
                                        }</TdEdura>
                                    <TdEdura className="w-35 text-end pe-1">{
                                        
                                        m.status === "hapus"?(
                                                <s className="text-rose-500 font-bold">{m.saldo && formatCurrency(m.saldo)}</s>
                                            ):(
                                                m.saldo && formatCurrency(m.saldo)
                                            )
                                            
                                    }</TdEdura>
                                    <TdEdura>{m.status?'Di hapus':''}</TdEdura>
                ...
                <tfoot>
                    <TRowEdura>
                        ...
                        <ThEdura className="text-end font-bold text-xl">{jumlahNOminalDihapus && formatCurrency(jumlahNOminalDihapus)||''}</ThEdura>
