...
export default function StatistikAbsensiBulananPage(){
    ...
    const data = useMemo(()=>{
            return  ormAbsen.getStatistikBulanan(tgl, isSabtuLibur);//.filter(s=>s.exist_in_this_month);
        }, 
    ...
    return (
            <div className="p-1" data-word="img">
                <h3 className="text-3xl text-center font-extrabold uppercase mb-0">Statistik Presensi Murid Bulanan</h3>
                <h4 className="text-2xl text-center font-extrabold uppercase mb-0">murid kelas {rombel}</h4>
                <h5 className="text-2xl text-center font-extrabold uppercase mb-7">{currentTapel({variant:'full'})} Semester {tgl?.getMonth()>5?1:2}</h5>
